...
// OTS-65: нужно ли крутить фоновый авто-рефреш (60с), чтобы поймать переход
// upcoming→live. Раньше поллили ТОЛЬКО когда матч УЖЕ идёт — но если юзер открыл
// страницу до старта (все матчи ещё upcoming), таймер не заводился вовсе, и когда
// матч начинался, страница оставалась протухшей: ● LIVE не появлялся без ручного
// F5. Симптом «я до сих пор не вижу матча в лайве». Теперь поллим, если матч ИДЁТ
// ИЛИ вот-вот стартует (kickoff в пределах +LEAD, или уже наступил, но апи ещё не
// перещёлкнул в live). Нижняя граница (-STALE) отсекает «зависший» upcoming, чтобы
// не поллить вечно на протухших данных у юзера с открытой вкладкой.
export function shouldPollForLive(matches, nowMs, leadMs = 15 * 60_000, staleMs = 6 * 3600_000) {
  return (matches || []).some((m) => {
    const s = Number(m.status);
    if (s >= 3 && s <= 7) return true;              // идёт сейчас
    if (s > 2) return false;                        // завершён — не поллим
    const t = new Date(m.dateTimeRaw).getTime();    // upcoming: ловим момент старта
    return Number.isFinite(t) && t <= nowMs + leadMs && t >= nowMs - staleMs;
  });
}
