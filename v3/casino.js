// ============================================================================
// ОТСОС · v3 · Режим казика 🎰 (OTS-87)
// Порт аттракциона из public/casino.js под DOM/классы v3. Клик по лого →
// музыка + эквалайзер в шапке + интро-сплэш «CASINO» + радужные бордеры карточек
// + canvas-поток монет. Кнопка «🎰 НАУГАД БЛЯ 🎰» на карточке крутит слот-машину
// (счёт двумя барабанами + скрамбл игрока) со звуком и ДЖЕКПОТОМ при равном счёте.
// Механика синтеза звука/физики монет — 1:1 из v2, DOM-хуки переписаны под v3.
// Уважает prefers-reduced-motion: без монет/интро/эквалайзера, но с музыкой и слотом.
// ============================================================================
...
let audioEl  = null;
let logoEl   = null;
let onToggle = null;   // (on:boolean) => void
let onJackpot = null;  // ({home,away,player}) => void — для тоста в лайв-тикер

export function isCasinoMode() { return casinoOn; }

// logo — элемент лого в шапке v3; cbs — { onToggle, onJackpot }
export function setupCasino(logo, cbs = {}) {
  logoEl = logo;
  onToggle = cbs.onToggle || null;
  onJackpot = cbs.onJackpot || null;
  if (!reduced) buildEqualizer();
}

export function toggleCasino() { casinoOn ? stopCasino() : startCasino(); }
...
function startCasino() {
  ...
  startMusic();
  if (!reduced) { showIntroSplash(); ensureAnalyser(); startEqualizer(); startCoins(); coinBurst(90); }
  onToggle?.(true);
}
function stopCasino() {
  casinoOn = false;
  ...
  stopMusic(); stopEqualizer(); stopCoins(); closeSlot();
  onToggle?.(false);
}

// ── Музыка ───────────────────────────────────────────────────────────────────
function startMusic() {
  if (!audioEl) { audioEl = new Audio(MUSIC_SRC); audioEl.loop = true; audioEl.volume = 0.55; }
  try { audioEl.currentTime = 0; } catch { /* not ready */ }
  audioEl.play().catch(() => { /* autoplay может требовать ещё жест */ });
}
function stopMusic() { if (audioEl) audioEl.pause(); }

// ── Синтез звука (WebAudio, без файлов) ──────────────────────────────────────
let audioCtx = null;
function ac() {
...
function playSpinSound(totalMs) {
  const ctx = ac(); if (!ctx) return;
  const start = ctx.currentTime, end = totalMs / 1000;
  blip(140, start, 0.08, "sawtooth", 0.18);
  let t = 0.05;
  while (t < end) { blip(820 + Math.random() * 160, start + t, 0.028, "square", 0.07); const p = t / end; t += 0.045 + p * p * 0.24; }
}
...
function buildEqualizer() {
  const bar = document.querySelector("header .hbar");
  const anchor = bar?.querySelector(".hspace");
  ...
  bar.insertBefore(wrap, anchor.nextSibling);
}
function ensureAnalyser() {
  const ctx = ac();
  if (!ctx || !audioEl || mediaSource) return;
  try {
...
function startEqualizer() {
  ...
  const cv = eqCanvas;
  cv.width = cv.clientWidth || 220; cv.height = cv.clientHeight || 40;
  const draw = () => {
    if (!casinoOn) return;
...
function resizeCoinCanvas() {
  ...
  coinW = window.innerWidth; coinH = window.innerHeight;
...
function makeCoin(origin) {
  ...
  return { x, y, vx, vy, rot: rnd(0, Math.PI * 2), vr: (Math.random() < 0.5 ? -1 : 1) * rnd(2, 9), size, glyph };
}
function coinFrame(ts) {
  ...
  if (coinSpawning) {
    ...
    while (coinSpawnAcc >= 1 && coins.length < cap) { coinSpawnAcc -= 1; coins.push(makeCoin(COIN_ORIGINS[coinOriginIdx++ % COIN_ORIGINS.length])); }
    if (coinSpawnAcc > 4) coinSpawnAcc = 0;
  }
  const ctx = coinCtx;
  ...
  for (let i = coins.length - 1; i >= 0; i--) {
    const p = coins[i];
    p.vy += COIN_GRAVITY * dt; p.x += p.vx * dt; p.y += p.vy * dt; p.rot += p.vr * dt;
    if (p.y > coinH + 80 || p.x < -120 || p.x > coinW + 120) { coins.splice(i, 1); continue; }
    const spr = coinSprites[p.glyph];
    ctx.save(); ctx.translate(p.x, p.y); ctx.rotate(p.rot);
    ctx.drawImage(spr, -p.size / 2, -p.size / 2, p.size, p.size); ctx.restore();
  }
}
...
function startCoins() { startCoinEngine(); coinSpawning = true; }
function pauseCoins() { coinSpawning = false; }
function resumeCoins() { if (casinoOn) coinSpawning = true; }
function stopCoins() {
  coinSpawning = false; coins = [];
  if (coinRAF) { cancelAnimationFrame(coinRAF); coinRAF = null; }
  if (coinCanvas) { window.removeEventListener("resize", resizeCoinCanvas); coinCanvas.remove(); coinCanvas = null; coinCtx = null; }
}
function coinBurst(n = 30) {
  if (!casinoOn || reduced) return;
  startCoinEngine();
  const { cap } = coinPerf();
  for (let i = 0; i < n && coins.length < cap; i++) coins.push(makeCoin(COIN_ORIGINS[i % COIN_ORIGINS.length]));
}

// ── Слот-машина ────────────────────────────────────────────────────────────────
// pool: [{name}] — состав для скрамбла имени. resolve({home,away,player}|null).
...
function buildStripHtml() {
  let html = "";
  for (let c = 0; c < REEL_CYCLES; c++) for (let d = 0; d < REEL_DIGITS; d++) html += `<div class="casino-reel-digit">${d}</div>`;
  return html;
}
...
function stopPlayerScramble() { if (playerScrambleTimer) { clearInterval(playerScrambleTimer); playerScrambleTimer = null; } }
function openSlot(home, away, player, pool, resolve) {
  ...
  if (pool.length) playerScrambleTimer = setInterval(() => { nameEl.textContent = pool[Math.floor(Math.random() * pool.length)].name || "…"; }, 70);
  ...
  const onLand = () => {
    ...
    if (jackpot) onJackpot?.({ home: String(home), away: String(away), player: player ? player.name : null });
  };
  ...
  const finish = (accepted) => {
    ...
    stopPlayerScramble(); closeSlot(); resumeCoins();
  ...
  overlay.querySelector(".casino-slot-backdrop").addEventListener("click", () => { if (landed >= 2) finish(false); });
}
function closeSlot() { stopPlayerScramble(); document.querySelector(".casino-slot-overlay")?.remove(); }
